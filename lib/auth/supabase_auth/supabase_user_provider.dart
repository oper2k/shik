...
class ShikMobileAppSupabaseUser extends BaseAuthUser {
  ShikMobileAppSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> shikMobileAppSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ShikMobileAppSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
